...
def load_model(args):
    ...
    model = LLM(model=args.ext_model_path, 
...
def load_ext_model(args):
    toker = AutoTokenizer.from_pretrained(
        args.ext_model_path,
        cache_dir=args.cache_dir,
        # use_auth_token='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',
    )
    model = LLM(model=args.ext_model_path, 
                dtype=torch.bfloat16,
                download_dir=args.cache_dir,
...
def generate_response(model, tokenizer, prompt, max_new_tokens=128, args=None):
    sampling_params = SamplingParams(
        temperature=0.0,
        max_tokens=max_new_tokens,
        stop=[tokenizer.eos_token],
    )
    # if 'instruct' in args.pretrained_model_path.lower():
    #     messages = [
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_model_path", type=str, default='Qwen/Qwen2-7B')
    parser.add_argument("--ext_model_path", type=str, default='meta-llama/Meta-Llama-3-8B-Instruct')
    ...
    parser.add_argument("--cache_dir", type=str, default='/mnt/nvme/guoqing/')
    ...
    ext_model, ext_toker =load_ext_model(args)
    ...
    try:
        for idx, data in tqdm(enumerate(datas[:])):
            ...
            initial_response = generate_response(model, toker, prompt, max_new_tokens=args.max_new_tokens, args=args).strip()
            init_extract_prompt = f'Please extract the final answer from the generation: {initial_response}'
            init_extract_response = generate_response(ext_model, ext_toker, init_extract_prompt, max_new_tokens=32, args=args).strip()
            print(f"initial extract resonse is: {init_extract_response}")
            data['initial_response'] = initial_response
            correct = data['answer']

            if correct.lower() not in init_extract_response.lower() or correct not in initial_response:
                ...
                while turn < max_turns:
                    ...
                    response = generate_response(model, toker, prompt, max_new_tokens=args.max_new_tokens, args=args)
                    extract_prompt = f'Please extract the final answer from the generation: {response}'
                    extracted_response = generate_response(ext_model, ext_toker, extract_prompt, max_new_tokens=32, args=args).strip()

                    prompt += f'\n\nModel: {response}'.rstrip()
                    turn += 1

                    if counter_option.lower() in extracted_response.lower() or counter_option.lower() in response.lower():
